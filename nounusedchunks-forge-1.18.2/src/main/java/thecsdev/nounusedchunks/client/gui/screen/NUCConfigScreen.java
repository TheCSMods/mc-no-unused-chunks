...
public class NUCConfigScreen extends Screen
{
	...
	protected void init()
	{
		...
		this.addRenderableWidget(btn_enabled = new Button(x2 - 160, y1 + 25, 150, 20,
				lt(Boolean.toString(NUCConfig.ENABLED)),
				arg0 ->
				{
		...
		this.addRenderableWidget(slider_ufc = new UnflagChanceSliderWidget(btn_enabled.x, btn_enabled.y + 25, btn_enabled.getWidth(), 20));
		
		//done
		this.addRenderableWidget(btn_done = new Button((this.width / 2) - 75, y3 + 5, 150, 20, tt("gui.done"), arg0 -> onClose()));
	}
	// --------------------------------------------------
	@Override
	public void render(PoseStack matrices, int mouseX, int mouseY, float delta)
	{
		...
		{
			if(child instanceof AbstractWidget && child.isMouseOver(mouseX, mouseY))
				((AbstractWidget)child).renderToolTip(matrices, mouseX, mouseY);
		});
	}
	// ==================================================
}